...
namespace Galantis\Toolkit\WPCLI;
...
class WPCLI {
	...
	public function index( $args, $assoc_args ) {
		if ( ! isset( $args[0] ) ) {
			WP_CLI::error( __( 'Please specify the post type which you want to index.', 'gala-toolkit' ) );
		}
		...
		if ( isset( $assoc_args['ids'] ) ) {
			...
			add_filter( 'gala_typesense_bulk_posts_per_page', array( __CLASS__, 'modify_posts_per_page' ) );
			$posted_data['offset'] = 0;
			$adminInstance         = Admin::getInstance();
			$response              = $adminInstance->bulkImportPosts( $posted_data, false );
			$count                 = 0;
			while ( 'success' === $response['status'] && $posted_data['offset'] <= $response['addInfo']['offset'] ) {
	...
	public function delete( $args, $assoc_args ) {
		if ( ! isset( $args[0] ) ) {
			WP_CLI::error( __( 'Please specify the post type which you want to delete.', 'gala-toolkit' ) );
		}
	...
	function register_commands() {
		WP_CLI::add_command( 'typesense index', array( $this, 'index' ) );
		WP_CLI::add_command( 'typesense health', array( $this, 'health' ) );
		WP_CLI::add_command( 'typesense delete', array( $this, 'delete' ) );
	}
}
