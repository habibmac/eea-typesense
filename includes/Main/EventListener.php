...
namespace Galantis\Toolkit\Main;
...
class EventListener {
	...
	public function __construct() {
		...
		// When posts added
		add_action( 'wp_after_insert_post', array( $this, 'postCreatedUpdatedHandler' ), 10, 3 );
		add_action( 'delete_post', array( $this, 'postDeletedHandler' ), 10, 2 );
		add_action( 'wp_trash_post', array( $this, 'postDeletedHandler' ), 10, 2 );

		// Run when category changed / updated
		add_action( 'saved_term', array( $this, 'onTermUpdated' ), 10, 4 );
	}
	...
	private function checkIfPostTypeEnabled( $post_type ): bool {
		...
		return in_array( $post_type, $this->enablePostTypes, true );
	}
	 ...
	 * @param bool $update
	 */
	public function postCreatedUpdatedHandler( int $post_ID, \WP_Post $post, bool $update ) { //phpcs:ignore
		if ( ! $this->checkIfPostTypeEnabled( $post->post_type ) ) {
			return;
		}

		$approvedPostStatus = apply_filters( 'gala_typesense_post_status', array( 'publish' ) );
		$removePostStatus   = apply_filters( 'gala_typesense_post_remove_status', array( 'draft', 'pending' ) );

		if (
			apply_filters( 'gala_typesense_force_remove_post_on_update', false, $post ) || ( is_array( $removePostStatus ) && in_array( $post->post_status, $removePostStatus, true ) ) ) {
			//you can use gala_typesense_post_force_remove_post_on_update to check for post meta values and remove it | if needed
			$this->typeSenseAPIInterface->deleteDocumentByID( $post->post_type, $post->ID );

			return;
		} elseif (
			is_array( $approvedPostStatus ) && ! in_array( $post->post_status, $approvedPostStatus, true )
		) {
			//can use gala_typesense_post_force_add_post_on_update to add posts via meta values
			return;
		}

		$supportedPostTypes = apply_filters( 'galantis_typesense_indexed_post_types', $this->enablePostTypes );
		...
		foreach ( $this->enablePostTypes as $enabledIndex ) {
			...
			if ( isset( $enabledIndexData['type'] ) && 'taxonomy' === $enabledIndexData['type'] ) {
	...
	public function onTermUpdated( $term_id, $tt_id, $taxonomy, $update ) {
		...
		$enabled_taxonomies = apply_filters( 'gala_typesense_enabled_taxonomy_for_post_type', array( 'category' ) );
