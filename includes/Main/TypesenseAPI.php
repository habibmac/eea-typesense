...
class TypesenseAPI {
	...
	public function getSchema( $name ) {
		...
		return apply_filters( 'gala_typesense_schema', $schema, $name );
	}
	...
	public function formatDocumentForEntry( $raw_data, int $object_id, string $schema_name ) {
		...
		if ( 'post' === $schema_name || ! isset( $available_post_types[ $schema_name ]['type'] )
			...
			foreach ( $fields as $field ) {
				switch ( $field['name'] ) {
					...
					case 'post_thumbnail_html':
						$formatted_data['post_thumbnail_html'] = has_post_thumbnail( $raw_data ) ? wp_get_attachment_image(
                            get_post_thumbnail_id( $raw_data ),
                            apply_filters( 'gala_typesense_html_image_size', 'medium' ),
                            false,
                            array(
					...
					case 'category':
						...
						$formatted_data['cat_link'] = is_array( $category_links ) && ! empty( $category_links ) ? array_values( array_unique( $category_links ) ) : array();
						$formatted_data['category'] = is_array( $category_names ) && ! empty( $category_names ) ? array_values( array_unique( $category_names ) ) : array();
						break;
					default:
						break;
				}
		...
		return apply_filters( 'gala_typesense_data_before_entry', $formatted_data, $raw_data, $object_id, $schema_name );
	}
	...
	public function bulkUpsertDocuments( $schemaIndex, array $documents ) {
		...
		if ( is_array( $documents ) ) {
			$documentsInJSONLFormat = implode(
				...
				/**
				 * @throws \JsonException
				 */                    static fn( array $document ) => wp_json_encode( $document, JSON_THROW_ON_ERROR ),
					$documents
				)
			);
		} else {
